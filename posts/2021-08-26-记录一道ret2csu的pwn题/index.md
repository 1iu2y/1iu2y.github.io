...
需要注意的是，题目说明了该题的远程环境为`Ubuntu18`，而我一开始是用`Ubuntu20`调试的，所以得到的偏移为`0x128`，多了`0x10`个字节，我说怎么一直不对，后来再装了个ubuntu18的wsl，发现果然如此，估计是系统的地址对齐之类的原因。有趣的是，在这一过程中还学习了一些wsl的新操作，也算是需求导向性学习了，[记录在了这里]()。
